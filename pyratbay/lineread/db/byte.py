...
class byte(dbdriver):
  def __init__(self, dbfile, pffile, log):
    ...
    ----------
    dbfile: String
       File with the Database info as given from BYTe.
    pffile: String
       File with the partition function.
    log: FILE
       A log file.
    """
    super(byte, self).__init__(dbfile, pffile)

    # Log file:
    self.log = log

    # Read states:
    sfile = dbfile[0:dbfile.rfind("__")] + ".states"
    with open(sfile, "r") as f:
      lines = f.readlines()
    nstates = len(lines)
    self.E       = np.zeros(nstates, np.double)  # State Energy
    self.g       = np.zeros(nstates, int)        # State statistical degeneracy
    ...
    self.mass     = [mass]
    self.isoratio = [1.0]   # FINDME: Remove?
    # Database name:
    self.name = "BYTe " + self.molecule
  ...
  def dbread(self, iwn, fwn, verb, *args):
    """
    Read a BYTe database (dbfile) between wavenumbers iwn and fwn.

    Parameters
    ----------
    dbfile: String
       A BYTe database filename.
    iwn: Float
    ...
    pt.msg(verb-4, "Process BYTe database between records {:,d} and {:,d}.".
    ...
    while (i < nread):
      ...
      # Extract values:
      upID[i], loID[i], A21[i] = line.split()
      ...
      if (i % interval) == 0.0  and  i != 0:
        ...
        gfval = self.g[loID[i]-1] * A21[i]*pc.C1/(8*np.pi*pc.c) / wn**2.0
