...
# Note
#
# You can also find this tutorial as a [Python script here](https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/opacity_line_sample.py) or as a [jupyter notebook here](https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/opacity_line_sample.ipynb).
#
# </div>
...
densities_solar = pa.ideal_gas_density(vmr_solar, pressure, temperature)
densities_super = pa.ideal_gas_density(vmr_super, pressure, temperature)
...
# Note
#
# While it's never a good idea to extrapolate,  it's allowed to extrapolate to lower
# pressures than those from the input table (assuming Doppler broadening dominates over
# Pressure broadening, and thus the line shapes should not change much at the lower pressures).
#
# But! it is **not possible** to extrapolate to higher pressures, because that
# requires a proper pressure broadening calculation, which needs to know the pressures.
#
# </div>
#
