...
# This tutorial shows how to fetch HITRAN/HITEMP line lists, and
# sample them into cross-section files for use in ``Pyrat Bay``
...
# The main issue with cross-section is that they are not too flexible
# (one might want to change, e.g., the wavelength resolution or line
# broadening parameters, for which the user would need to re-generate
# cross-sections from the TLI files).  For this reason ``Pyrat Bay``
# was designed with this two-step approach.
...
# For this demo, we will get the HITRAN/H2O and HITEMP/CO line
# lists. We can do this with the following prompt commands:
...
# the command line.  The config file below
# (https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/tli_hitran_H2O_cookbook.cfg)
# converts the HITRAN/H2O line-list (see ``dblist``) into a TLI file
# (see ``tlifile`` or ``logfile``).

# Partition-function information must also be provided (see
# ``pflist``).  If you set ``pflist=tips``, ``Pyrat Bay`` will use the
# partition functions from [Gamache et
# al. (2017)](https://ui.adsabs.harvard.edu/abs/2017JQSRT.203...70G).
# Alternatively, one can input the path to a partition-function file
# [TBD: Explain how].

# Lastly, the user can specify the wavelength range of the extracted
# data (see ``wllow`` and ``wlhigh``).  Normally one set the widest
# know range (to avoid needing to re-calculating TLI files if a future
# calculation needs it), but for sake of this demo, we will extract
# just over the region that we need:
...
# (https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/tli_hitran_CO_cookbook.cfg):
...
# (https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/opacity_hitran_H2O_cookbook.cfg)
...
#   $\Delta$wavenumber, units in cm$^{-1}$)
  
# For the composition (`species`), make sure to include the molecule
# for which we are computing the cross-sections. Also, include the
# *background* gas, which is relevant for the pressure broadening
# (here, we assume a H2/He-dominated atmosphere).  Only the VMR values
# of the background gasses are important, trace-gas VMRs are
# irrelevant (see `chemistry` or `uniform`. `tmodel` and `tpars` are
# needed to define the atmosphere's temperature profile, but for an
# opacity run, these do not impact the calculations.
...
# (https://github.com/pcubillos/pyratbay/blob/master/docs/cookbooks/opacity_hitran_CO_cookbook.cfg):
...
# To generate the cross-section files, we run these ``Pyrat Bay``
# prompt commands:
