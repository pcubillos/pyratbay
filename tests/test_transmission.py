...
def test_transmission_sampled_cs(tmp_path):
    cfg = make_config(
        tmp_path,
        ROOT+'tests/configs/spectrum_transmission_test.cfg',
...
def test_transmission_alkali(tmp_path):
    ...
    pyrat = pb.run(cfg)
    spectrum = pyrat.spec.spectrum
    np.testing.assert_allclose(spectrum, expected['alkali'], rtol=rtol)
...
def test_transmission_fit(tmp_path):
    ...
    pyrat = pb.run(cfg)
    spectrum = pyrat.spec.spectrum
    np.testing.assert_allclose(spectrum, expected['fit1'], rtol=rtol)
    ...
    models = [model.name for model in pyrat.opacity.models]
    i_deck = models.index('deck')
    cloud_deck = pyrat.opacity.models[i_deck]
...
def test_opacity_reset_wn(tmp_path, wl_low, wl_high):
    ...
    etab = expected['sampled_cs'][wn_range]
...
def test_transmission_skip_kurucz_rayleigh(tmp_path):
    reset = {
        'rayleigh': 'rayleigh_H2 rayleigh_He',
    }
    cfg = make_config(
